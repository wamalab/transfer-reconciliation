...
This page shows all transactions that have been matched on a one-to-one basis.
...
```js
const uniqueMatches = new Set(matches1to1.map(d => d.index_out)).size;
const totalAmount = matches1to1.reduce((sum, d) => sum + Math.abs(d.amount_out), 0);
...
<div class="grid grid-cols-3">
  ...
  <div class="card">
    <h2>Total Amount</h2>
    <span class="big">$${(totalAmount / 1000000).toFixed(1)}M</span>
  </div>  <div class="card">
    <h2>Average Amount</h2>
    <span class="big">$${(avgAmount / 1000000).toFixed(1)}M</span>
  </div>
</div>
...
```js
Inputs.table(tableData, {  columns: ["index", "match_id", "date_out", "amount_out", "account_id_out", "date_in", "amount_in", "account_id_in"],
  header: {
    index: "#",
    match_id: "Match ID",
    date_out: "Out Date",
